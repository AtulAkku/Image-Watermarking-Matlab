...
for channel = 1:3
    ...
    % Calculate Poisson noise with desired lambda value
    lambda = 0.1;  % Adjust this value
    noise_channel = poissrnd(lambda, size(original_image(:,:,channel)));

    % Add noise to the watermarked image
    watermarked_image_double_channel = watermarked_image_double_channel + noise_channel / 255; % Scale to [0, 1]
...
% Display results
fprintf('==========================\n');
fprintf('Poisson Noise Parameters:\n');
fprintf('Lambda: %f\n', lambda);
fprintf('==========================\n');
