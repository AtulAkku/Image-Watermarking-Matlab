% Take user input for original image
original_image_path = input('Enter the path of the original image: ', 's');
original_image = imread(original_image_path);

% Take user input for watermark image
watermark_image_path = input('Enter the path of the watermark image: ', 's');
watermark_image = imread(watermark_image_path);

% Get the size of the images
[Mo, No, ~] = size(original_image);

% Take user input for wavelet type
waveletType = input('Enter the wavelet type (e.g., db4, haar): ', 's');

% Resize the watermark image to match the original image size
watermark_image = imresize(watermark_image, [Mo, No]);

% Take user input for alpha value
alpha = input('Enter the scaling factor (alpha) for the watermark: ');

% Apply DWT to each color channel of the original image and the watermark image
for k = 1:3 % Iterate over RGB color channels
    [cA1, cH1, cV1, cD1] = dwt2(original_image(:,:,k), waveletType);
